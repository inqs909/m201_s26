...
gg1 <- seq(0, 1, length.out = 1000) |> 
  dbeta(shape1 = 2, shape2 = 5) |> 
  tibble(x = seq(0, 1, length.out = 1000), y = _) |>
  ggplot(aes(x, y)) + geom_line() + 
  ggtitle("DGP") + ylab("Density") +
  labs(alt = "A density plot displaying a right-skewed distribution from the values 0 to 1.") 

gg2 <- rbinom(200000, size = 20, prob = 1/5) |> 
  tibble(y = _) |>
  ggplot(aes(y)) + geom_bar() + 
  ggtitle("DGP") + ylab("Frequency") +
  labs(alt = "A bar chart displaying the frequency of data points between 0 to 10. The most common number is 4") 
...
gg3 <- gg3 +
  labs(alt =  "There are 2 side-by-side plots indicating the Data Generating Process (left) and sample plot (right). The sample plot contains a density plot showing how the data is similar to the DGP.")
...
```{r}
#| echo: false
#| fig-alt: !expr ggplot2::get_alt_text(gg1)
#| message: false
...
```{r}
#| echo: false
#| fig-alt: !expr ggplot2::get_alt_text(gg2)
...
```{r}
#| echo: false
#| fig-alt: !expr ggplot2::get_alt_text(gg3)
