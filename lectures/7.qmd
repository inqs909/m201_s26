...
Find the numerical statistics of `body_mass` for each penguins `species`, for the `penguins` data set.
...
Find the descriptive statistics for each `disease` based on the `thal` condition in the `heart_disease` data set.

::: fragment

```{r}
#| code-fold: false
cat_stats(heart_disease$disease, heart_disease$thal)
```
...
On average, CAT has a larger/smaller Y than REF by about $\beta_1$.
