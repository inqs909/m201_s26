...
editor: source
image: img/cat.png
image-alt: "A stacked bar plot shoing the counts for each penguin species and their respective island they live on."
knitr:
  opts_chunk: 
    echo: true
    eval: true
...
```{r}
#| fig-alt: |
#|  A bar chart showing the frequency of each caffeinated coffee type: 
#|  Decaf, Full Caffeine, Half calf, and NA. Th most common type is Full Caffeine.
...
```{r}
#| fig-alt: |
#|  A bar chart showing the proportion of each caffeinated coffee type: 
#|  Decaf, Full Caffeine, Half calf, and NA. Th most common type is Full Caffeine.
...
**Example (horizontal):**
```{r}
#| fig-alt: A bar plot showing the caffeine levels by different tastes. The bars are layed horizontal instead of vertical.
...
```{r}
#| fig-alt: A bar plot showing the caffeine levels by different tastes. The different tastes are side-by-side instead of stacked.
ggplot(coffee, aes(x = caffeine, fill = taste)) +
geom_bar(position = position_dodge())
```
...
**Example (horizontal):**
```{r}
#| fig-alt: A bar plot showing the caffeine levels by different tastes. The different tastes are side-by-side instead of stacked. The bars are positioned horizontally.
ggplot(coffee, aes(y = caffeine, fill = taste)) +
geom_bar(position = position_dodge())
```
...
**Example:**
```{r}
#| fig-alt: A pie chart displaying the how common each caffeine type. 
