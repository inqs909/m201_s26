...
$H_1$: There is a relationship between heart disease probability and resting blood pressure, adjusting for chest pain ($\beta_{bp} \ne 0$)
...
## Penguins Bill Depth vs Flipper Length

```{r}
#| fig-alt: |
#|   The plot shows a scatter plot between penguin bill depth versus flipper length. 
#|   A line is overlayed on the plot indicating a negative relationship.
penguins |> 
  ggplot(aes(bill_dep, flipper_len)) +
  geom_point() +
  stat_smooth(method = "lm")
```

## Penguins Bill Depth vs Flipper Length by Species

```{r}
#| fig-alt: |
#|   The plot shows a scatter plot between penguin bill depth versus flipper length color-coded by species. 
#|   Three each species has a line are overlayed indicating a positive relationship.

penguins |> 
  ggplot(aes(bill_dep, flipper_len, color = species)) +
  geom_point()+
  stat_smooth(method = "lm")
...
## Penguins: Residual vs Fitted Plots
...
## Heart: Residual vs Fitted Plots
