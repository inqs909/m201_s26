...
$$
lo(Y) =  \beta_0 + \beta_1 D_1 + \cdots + \beta_p D_p
$$
...
- With both ANOVA an LRT, we only know if there is one group pairing are different from each other.

- We want to know which group pairing are different.

- This can be done with multiple hypothesis testing.

- However, we need to overcome some challenges.
...
- $H_0$: The mean of group A is the same of group B.

- $H_1$: The mean of group A is different of group B.

## MCT: Hypothesis Proportion

- $H_0$: The proportion of group A is the same of group B.

- $H_1$: The proportion of group A is different of group B.
